...
if(nrow(bbs_data) == 0L){
  ...
  bbs_data <-
    ...
    dplyr::mutate(
      ...
      low_confidence = ifelse(grepl('Low confidence', WARNINGS), 1, 0),
            WARNINGS = gsub('Low confidence', '', ifelse(is.na(WARNINGS),  '', WARNINGS)),
...
if(params$params_list$deploymentSummary){
  ...
  invisible(
    webshot::webshot("deployment_map.html", file = "deployment_map.png")
  )
}
```
  ...
  lapply(function(x){
    y <-
      ...
      dplyr::summarise(
        ...
        #number.low.confidence = sum(low_confidence)
      ) %>%
      dplyr::ungroup() %>%
      ...
      dplyr::summarize(
        ...
        WARNINGS = paste0(number.of.records, ' records with "', WARNINGS, '" warning', collapse = ';')) %>%
      dplyr::ungroup() %>%
      dplyr::rowwise() %>%
      dplyr::mutate(
      ...
      #   WARNINGS = ifelse(number.low.confidence > 0, 
      #                     ifelse(WARNINGS == '', paste(number.low.confidence, 'records with "Low confidence" warning'), paste(WARNINGS, paste(number.low.confidence, 'records with "Low confidence" warning'), sep = '; ')), WARNINGS
      # )) %>%
      ) %>%
